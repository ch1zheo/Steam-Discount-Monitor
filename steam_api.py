...
# Используется публичный поисковый эндпоинт Steam Store (тот же,
# что использует сайт store.steampowered.com/search). Он отдаёт ПОЛНЫЙ
# список игр со скидкой, в отличие от featurecategories API, где всего
# ~50 "витринных" позиций.

# Официального документированного API для этого у Valve нет, поэтому
# разметка страницы может со временем поменяться — если парсинг
# перестанет находить игры, в первую очередь проверяйте CSS-селекторы
# в функции _parse_results_html.
...
def fetch_discounted_games(cc: str, lang: str = "russian",
